...
regardless of which target they are on. For example, with
\texttt{data\_replicas=2}, \texttt{foreground\_target=ssd}, and
\texttt{background\_target=hdd}:

\begin{itemize}
\item Data is initially written to the SSD target
\item The background mover copies it to the HDD target
\item Since both SSD and HDD copies have \texttt{durability=1} by default,
  each counts as one replica---the two copies together satisfy the replica
  requirement
\item To use the SSDs as a pure write cache (data evictable once on HDD),
  set \texttt{durability=0} on the SSD devices; the allocator will then
  write two durable copies on the HDD target
\end{itemize}
